...
%
\begin{flalign*}
  g(n) &= \llbracket \T{rec}(xs, \T{Nil} \mapsto \LP 1,\T{Cons} \LP x, \T{Nil} \RP\RP, \\
       &\quadsix \T{Cons}  \mapsto \LP y, \LP ys,r \RP \RP .\LP 1 + r_c, \T{Cons} \LP y,r_p\RP\RP)\rrbracket \{xs \mapsto n, x \mapsto x\}\\
       &= \bigvee\limits_{size\ z \leq n} case(z, f_{Nil}, f_{Cons}) \\
  \text{where}& \\
  f_{Nil}(\ast) &= \llbracket \LP 1,\T{Cons} \LP x, \T{Nil} \RP\RP \rrbracket \{xs \mapsto n, x \mapsto x\}\\
             &= (1, 1) \\
  f_{Cons}(1, m) &= \llbracket \LP 1 + r_c, \T{Cons} \LP y,r_p\RP\RP \rrbracket \{xs \mapsto n, x \mapsto x, y \mapsto 1, ys \mapsto m, r \mapsto g(m) \} \\
                 &= (1 + g_c(m), 1 + g_p(m))
\end{flalign*}
%
...
%
\begin{flalign*}
  g(n) &= \llbracket \T{rec}(xs, \T{Nil} \mapsto \LP 1,\T{Nil}\RP, \\
       &\quadfive \T{Cons} \mapsto \LP x, \LP xs',r\RP \RP. (3 + r_c) +_c (\|\T{snoc}\|_p\ r_p)_p\ x)\rrbracket \{xs \mapsto n\} \\
       &= \bigvee\limits_{size\ z \leq n} case(z, f_{Nil}, f_{Cons}) \\
       &\text{where}\\
  f_{Nil}(\ast) &= \llbracket \LP 1,\T{Nil}\RP\rrbracket \{xs \mapsto n\} \\
                &= (1, 0) \\
