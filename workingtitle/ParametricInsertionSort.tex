...
%
\begin{flalign*}
  \llbracket \T{rec}&(((f\ x)_p\ y)_p, \T{True} \mapsto \LP 1, \T{Cons}\LP x, \T{Cons}\LP y,ys\RP\RP\RP, \T{False}\mapsto \LP 1 + r_c,\T{Cons}\LP y,r_p\RP\RP)\rrbracket \xi \\
  ...
  f_{True}(\ast) &= \llbracket \LP 1, \T{Cons}\LP x, \T{Cons}\LP y,ys\RP\RP\RP \rrbracket\xi \\
                 &= (1, \llbracket \T{Cons}\LP x, \T{Cons}\LP y,ys\RP\RP\rrbracket)\xi \\
                 &= (1, (max(x,j), 2 + m)) \\
  f_{False}(\ast) &= \llbracket \LP 1 + r_c,\T{Cons}\LP y,r_p\RP\RP\rrbracket \xi \\
...
%
\begin{flalign*}
  g(i, n) &= \llbracket\T{rec}(xs, \T{Nil} \mapsto \LP 1,\T{Cons}\LP x,\T{Nil}\RP\RP, \\
            &\quad \T{Cons}\mapsto \LP y, \LP ys,r \RP\RP. (3 + (f x)_c) \\
            &\quadthree +_c \T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \LP 1, \T{Cons}\LP x,\T{Cons}\LP y,ys\RP\RP\RP, \\
            &\quadsix \T{False}\mapsto \LP 1 + r_c, \T{Cons}\LP y,r_p\RP\RP))\rrbracket \xi \\
            &\text{where} \\
  \xi &= \{xs \mapsto (i, n), x \mapsto x\} \\
  g(i,n) &= \bigvee\limits_{size(z) \leq (i,n)} case(z, f_{Nil}, f_{Cons}) \\
         &\text{where}\\
  f_{Nil}(\ast) &= \llbracket \LP 1,\T{Cons}\LP x,\T{Nil}\RP\RP \rrbracket \xi \\
                &= (1, (x, 1)) \\
  f_{Cons}(j,m) &= \llbracket (4 + (f\ x)_c) +_c \T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \LP 1, \T{Cons}\LP x,\T{Cons}\LP y,ys\RP\RP\RP, \\
                &\quadthree \T{False}\mapsto \LP 1 + r_c, \T{Cons}\LP y,r_p\RP\RP)\rrbracket \xi \{ y \mapsto j, ys \mapsto (j, m), r \mapsto g(j, m)\} \\
                &= (4 + (f\ x)_c) \pplus_c \llbracket\T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \LP 1, \T{Cons}\LP x,\T{Cons}\LP y,ys\RP\RP\RP, \\
                &\quadthree \T{False}\mapsto \LP 1 + r_c, \T{Cons}\LP y,r_p\RP\RP)\rrbracket \xi \{ y \mapsto j, ys \mapsto (j, m), r \mapsto g(j, m)\} \\
...
\begin{flalign*}
  g(i, n) &= \llbracket \T{rec}(xs, \T{Nil} \mapsto \LP 1,\T{Nil} \RP,  \\
          &\quadthree \T{Cons} \mapsto \LP y, \LP ys,r \RP\RP.(4 + r _c) +_c ((\|\T{insert}\|_p f)_p y)_p r_p) \rrbracket \{ xs \mapsto (i, n), f\mapsto f\} \\
          &= \bigvee\limits_{size(z)\leq n} case(z, f_{Nil}, f_{Cons}) \\
  %
          &\text{where}\\
  f_{Nil} &= \llbracket \LP 1,\T{Nil}\RP \rrbracket \{ xs \mapsto (i,n)\} \\
          &= (1, (-\infty,0)) \\
