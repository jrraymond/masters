...
\begin{flalign*}
  \|\T{insert}\ f\ x\ xs\| &= (1 + \|\T{insert}\ f\ x\|_c + \|xs\|_c) +_c \|\T{insert}\ f\ x\|_p \|xs\|_p \\
     ...
     &\quadfour \T{Cons}\mapsto \langle y,\langle ys,r\rangle\rangle.(3 + ((\|f\|_p \|x\|_p)_p  y)_c) \\
     &\quadsix +_c \T{rec}((\|f\|_p \|x\|_p)_p  y)_p, \\
     &\quadeight \T{True}\mapsto \langle 1, \T{Cons}\langle \|x\|_p,\T{Cons}\langle y,ys\rangle\rangle\rangle \\
     &\quadeight \T{False}\mapsto \langle 1 + r_c,\T{Cons}\langle y,r_p\rangle\rangle))
\end{flalign*}
%
% END INSERT APPLIED
%
...
\begin{flalign*}
  \|\T{sort}\| &= \| \lambda f.\lambda xs.\T{rec}(xs, \T{Nil} \mapsto \T{Nil}, \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.\T{insert}\ f\ y\ \T{force}(r))\| \\
               &= \langle 0, \lambda f.\langle 0,\lambda xs.\|\T{rec}(xs, \T{Nil} \mapsto \T{Nil}, \\
               &\quadthree \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.\T{insert}\ f\ y\ \T{force}(r))\|\rangle\rangle \\
               &= \langle 0, \lambda f.\langle 0,\lambda xs.\|xs\|_c +_c \T{rec}(\|xs\|_p, \T{Nil} \mapsto 1 +_c \|\T{Nil}\|, \\
               &\quadthree \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.1 +_c \|\T{insert}\ f\ y\ \T{force}(r)\|)\rangle\rangle \\
               &\text{As $xs$ is a variable, $\|xs\| = \langle 0,xs \rangle$.} \\
               &\text{We have seen before $\|\T{Nil}\| = \langle 0,\T{Nil}\rangle$.}\\
               &= \langle 0, \lambda f.\langle 0,\lambda xs.\T{rec}(xs, \T{Nil} \mapsto 1 +_c \langle 0,\T{Nil}\rangle, \\
               &\quadthree \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.1 +_c \|\T{insert}\ f\ y\ \T{force}(r)\|)\rangle\rangle \\
               &\text{We can use our translation of \T{insert} applied to three arguments.} \\
               &= \langle 0, \lambda f.\langle 0,\lambda xs.\T{rec}(xs, \T{Nil} \mapsto 1 +_c \langle 0,\T{Nil}\rangle, \\
               &\quad \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.(4 + \|f\|_c + \|y\|_c + \|force(r)\|_c) \\
               &\quadthree +_c ((\|\T{insert}\|_p \|f\|_p)_p \|y\|_p)_p \|force(r)\|_p)\rangle\rangle \\
               &\text{The variables $f$, $y$, and $r$ translate to $\langle 0,f\rangle$, $\langle 0,y\rangle$, and $\langle 0,r\rangle$ respectively.}\\
               &\text{The expression $\T{force}(r)$ translates to $\|r\|_c +_c \|r\|$, which is just $r$.} \\
               &= \langle 0, \lambda f.\langle 0,\lambda xs.\T{rec}(xs, \T{Nil} \mapsto 1 +_c \langle 0,\T{Nil}\rangle, \\
               &\quad \T{Cons} \mapsto \langle y,\langle ys,r \rangle\rangle.(4 + r_c) +_c ((\|\T{insert}\|_p f)_p y)_p r_p)\rangle\rangle
\end{flalign*}
%
...
%
\begin{flalign*}
  \llbracket \T{rec}&(((f\ x)_p\ y)_p, \T{True} \mapsto \langle 1, \T{Cons}\langle x, \T{Cons}\langle y,ys\rangle\rangle\rangle, \T{False}\mapsto \langle 1 + r_c,\T{Cons}\langle y,r_p\rangle\rangle)\rrbracket \xi \\
  ...
  f_{True}(\ast) &= \llbracket \langle 1, \T{Cons}\langle x, \T{Cons}\langle y,ys\rangle\rangle\rangle \rrbracket\xi \\
                 &= (1, \llbracket \T{Cons}\langle x, \T{Cons}\langle y,ys\rangle\rangle\rrbracket)\xi \\
                 &= (1, (max(x,j), 2 + m)) \\
  f_{False}(\ast) &= \llbracket \langle 1 + r_c,\T{Cons}\langle y,r_p\rangle\rangle\rrbracket \xi \\
...
%
\begin{flalign*}
  g(i, n) &= \llbracket\T{rec}(xs, \T{Nil} \mapsto \langle 1,\T{Cons}\langle x,\T{Nil}\rangle\rangle, \\
            &\quad \T{Cons}\mapsto \langle y, \langle ys,r \rangle\rangle. (3 + (f x)_c) \\
            &\quadthree +_c \T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \langle 1, \T{Cons}\langle x,\T{Cons}\langle y,ys\rangle\rangle\rangle, \\
            &\quadsix \T{False}\mapsto \langle 1 + r_c, \T{Cons}\langle y,r_p\rangle\rangle))\rrbracket \xi \\
            &\text{where} \\
  \xi &= \{xs \mapsto (i, n), x \mapsto x\} \\
  g(i,n) &= \bigvee\limits_{size(z) \leq (i,n)} case(z, f_{Nil}, f_{Cons}) \\
         &\text{where}\\
  f_{Nil}(\ast) &= \llbracket \langle 1,\T{Cons}\langle x,\T{Nil}\rangle\rangle \rrbracket \xi \\
                &= (1, (x, 1)) \\
  f_{Cons}(j,m) &= \llbracket (4 + (f\ x)_c) +_c \T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \langle 1, \T{Cons}\langle x,\T{Cons}\langle y,ys\rangle\rangle\rangle, \\
                &\quadthree \T{False}\mapsto \langle 1 + r_c, \T{Cons}\langle y,r_p\rangle\rangle)\rrbracket \xi \{ y \mapsto j, ys \mapsto (j, m), r \mapsto g(j, m)\} \\
                &= (4 + (f\ x)_c) \pplus_c \llbracket\T{rec}(((f\ x)_p\ y)_p, \T{True}\mapsto \langle 1, \T{Cons}\langle x,\T{Cons}\langle y,ys\rangle\rangle\rangle, \\
                &\quadthree \T{False}\mapsto \langle 1 + r_c, \T{Cons}\langle y,r_p\rangle\rangle)\rrbracket \xi \{ y \mapsto j, ys \mapsto (j, m), r \mapsto g(j, m)\} \\
                &= (4 + (f\ x)_c) \pplus_c ((1, (max(x,j), 2 + m)) \\
...
%
\begin{flalign*}
  \|\T{insert}\| &= (0, \llambda f.(0,\llambda x.(0,\llambda (i,n). g(i,n)))) \\
  \text{where}& \\
  g(i,n) &= \bigvee\limits_{size(z) \leq (i,n)} case(z, f_{Nil}, f_{Cons}) \\
         &\text{where}\\
  f_{Nil}(\ast) &= (1, (x, 1)) \\
  f_{Cons}(j,m) &= (4 + (f\ x)_c) \pplus_c ((1, (max(x,j), 2 + m)) \\
...
\begin{flalign*}
  g(i, n) &= \llbracket \T{rec}(xs, \T{Nil} \mapsto \langle 1,\T{Nil} \rangle,  \\
          &\quadthree \T{Cons} \mapsto \langle y, \langle ys,r \rangle\rangle.(4 + r _c) +_c ((\|\T{insert}\|_p f)_p y)_p r_p) \rrbracket \{ xs \mapsto (i, n), f\mapsto f\} \\
          &= \bigvee\limits_{size(z)\leq n} case(z, f_{Nil}, f_{Cons}) \\
  %
          &\text{where}\\
  f_{Nil} &= \llbracket \langle 1,\T{Nil}\rangle \rrbracket \{ xs \mapsto (i,n)\} \\
          &= (1, (-\infty,0)) \\
f_{Cons} &= \llbracket (4 + r _c) +_c ((\|\T{insert}\|_p f)_p y)_p r_p) \rrbracket \xi \\
         &\text{where}\quad \xi = \{xs \mapsto (i,n), f \mapsto f, y \mapsto j, ys \mapsto (j,m), r \mapsto g(j,m)\} \\
         &= (4 + g_c(j,m)) \pplus_c ((\llbracket\|\T{insert}\|_p\rrbracket\xi\ f)_p j)_p g_p(j,m) \\
         &= (5 + g_c(j,m)) \pplus_c (((f\ j)_p\ j)_c g_p(j,m), (max\{j,j\},g_p(j, m)+1)) \\
