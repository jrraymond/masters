...
%In order to reap the benefits of using a tree instead of a list, we need to
%assume the tree is balanced. We will use the same notion of balance as AVL
%trees. The height of a tree is defined as:
%%
%\begin{align*}
%  height(E) &= 0\\
...
%between the left and right subtrees of a node.  If a node has $n$ sub-nodes,
...
%of the node, that is, $h \leq 2log_2(n) + 2$. Then in the worst case, one of the
...
%height are maximized.
%
%\paragraph{}
%The minimum number of nodes in a subtree of height $h$ is
...
%%\[
%%  x(n) = -1+\frac{1}{2}\left(\frac{1}{2}+\frac{1}{2}\sqrt 5 \right)^n+\frac{3\sqrt 5}{10}\left(\frac{1}{2}+\frac{1}{2}\sqrt 5\right)^n - \frac{3\sqrt 5}{10}\left(\frac{1}{2}-\f
%%\]
%%The lower bound on this recurrence is order $k^h$ for some $k$.
%
%\paragraph{}
%A tree with the maximum number of nodes is a \textit{perfect tree}. The number
%of nodes in a perfect tree of height $h$ is $2^h-1$.
%
%\paragraph{}
%So the largest difference between number of nodes is $2^{h-1} - 1 -
%min\_nodes(h-2)$.
%
%\paragraph{}
%Now in our analysis of the span, we can use the fact that $n_0 - n_1 \leq
%2^{h-1} - 1 - min\_nodes(h-2)$ where $h$ is the height of the current node. We
%will make the assumption that the cost of $g$ is monotonically increasing, that
%is larger inputs result in larger outputs.
%%
%\begin{lemma}
%	$\pi_1 g_c(i,n) \leq (2 + (f\ i)_c) 2 log_2 n$
%\end{lemma}
%%
%\begin{proof}
%\begin{align*}
...
%                  &= 1 \vee \pi_1 (\bigvee\limits_{\substack{1 + n_0 + n_1 \leq n \\ max(j,j_0,j_1) \leq i}} f_N(j,(j_0,n_0),(j_1,n_1)))_c \\
%                  &= 1 \vee \bigvee\limits_{\substack{1 + n_0 + n_1 \leq n \\ max(j,j_0,j_1) \leq i}} 2 + max((f\ j)_c,\pi_1 g_c(j_0,n_0),\pi_1 g_c(j_1,n_1)) \\
%                  &= \bigvee\limits_{\substack{1 + n_0 + n_1 \leq n \\ max(j,j_0,j_1) \leq i \\ n_0 \geq n_1 \\ n_0 - n_1 \leq 2^{h-1} - 1 - min\_nodes(h-2)}} 2 + (f\ j)_c \vee \pi_1 g_c(j_0,n_0))\\
%                  &= 2 + (f\ j)_c \vee \pi_1 g_c(i,hj))\\
%\end{align*}
%\end{proof}
